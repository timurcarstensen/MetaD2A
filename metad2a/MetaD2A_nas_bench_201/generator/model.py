...
class GeneratorModel(nn.Module):
    def __init__(self, args, graph_config):
        ...
        self.decoding_gate = nn.Sequential(
            nn.Linear(self.vs, self.hs), nn.Sigmoid()
        )
        ...
        self.set_fc = nn.Sequential(nn.Linear(512, self.nz), nn.ReLU())
    ...
    def _get_zeros(self, n, length):
        return torch.zeros(n, length).to(
            self.get_device()
    ...
    def _propagate_to(
        ...
        if reverse:
            ...
            if self.vid:
                vids = [
                    self._one_hot(g.successors(v), self.max_n) for g in G
                ]
            ...
            if self.vid:
                vids = [
                    self._one_hot(g.predecessors(v), self.max_n)
                    for g in G
                ]
            if gate is None:
    ...
    def graph_decode(self, z, stochastic=True):
        ...
        H0 = self.tanh(
            self.fc3(z)
        )  # or relu activation, similar performance
        ...
        for idx in range(1, self.max_n):
            ...
            if (
                idx == self.max_n - 1
            ):  # force the last node to be end_type
                ...
                if stochastic:
                    type_probs = (
                        F.softmax(type_scores, 1).cpu().detach().numpy()
                    )
                    new_types = [
        ...
        for g in G:
            del g.vs[
                "H_forward"
            ]  # delete hidden states to save GPU memory
        return G
    ...
    def loss(self, mu, logvar, G_true, beta=0.005):
        ...
        for v_true in range(1, self.max_n):
            ...
            idx1 = [
                i for i, x in enumerate(true_edges) for _ in range(len(x))
            ]
