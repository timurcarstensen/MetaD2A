...
class Generator:
    ...
    def meta_train_epoch(self, epoch):
        ...
        for x, g, acc in tqdm(self.mtrloader):
            ...
            self.mtrlogger.accum(
                [loss.item() / cnt, recon.item() / cnt, kld.item() / cnt]
            )
    ...
    def generate_architectures(
        ...
        with torch.no_grad():
            for x in meta_test_loader:
                ...
                for g in generated_graph_lst:
                    architecture_string = (
                        decode_igraph_to_NAS_BENCH_201_string(g)
                    )
                    total_cnt += 1
                    if architecture_string is not None:
                        if (
                            architecture_string
                            not in architecture_string_lst
                        ):
                            valid_cnt += 1
                            architecture_string_lst.append(
                                architecture_string
                            )
        ...
        with open(spath, "w") as f:
            ...
            print(msg)
            f.write(msg + "\n")
            for i, architecture_string in enumerate(
                architecture_string_lst
            ):
