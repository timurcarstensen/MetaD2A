...
def get_datasets(name, root, cutout, use_num_cls=None):
    ...
    class_num = (
        Dataset2Class[name] if use_num_cls is None else len(use_num_cls)
    )
...
def get_nas_search_loaders(
    ...
    if dataset == "cifar10":
        ...
        if not os.path.exists(
            "{:}/{}-test-split.txt".format(config_root, dataset)
        ):
            import json
            ...
            split = {
                "xvalid": ["int", strlist[: len(valid_data) // 2]],
                "xtest": ["int", strlist[len(valid_data) // 2 :]],
            }
            with open(
                "{:}/{}-test-split.txt".format(config_root, dataset), "w"
            ) as f:
