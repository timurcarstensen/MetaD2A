...
from metad2a.MetaD2A_nas_bench_201.nas_bench_201.functions import (  # noqa: E402
    evaluate_for_seed,
)
from metad2a.MetaD2A_nas_bench_201.nas_bench_201.log_utils import (  # noqa: E402
    AverageMeter,
    Logger,
    convert_secs2time,
)
from metad2a.MetaD2A_nas_bench_201.nas_bench_201.models import (  # noqa: E402
    CellArchitectures,
    CellStructure,
    get_search_spaces,
)
from metad2a.MetaD2A_nas_bench_201.nas_bench_201.procedures import (  # noqa: E402
    get_machine_info,
)
...
def evaluate_all_datasets(
    ...
    for dataset, xpath, split in zip(datasets, xpaths, splits):
        ...
        if dataset in ["mnist", "svhn", "aircraft", "pets"]:
            if use_less:
                config_path = (
                    "nas_bench_201/configs/nas-benchmark/LESS.config"
                )
            else:
            ...
            if not os.path.exists(p):
                ...
                strlist = [
                    str(label_list[i]) for i in range(len(label_list))
                ]
                splited = {
        ...
        logger.log(
            "Evaluate ||||||| {:10s} ||||||| Config={:}".format(
                dataset_key, config
            )
        )
        for key, value in ValLoaders.items():
            logger.log(
                "Evaluate ---->>>> {:10s} with {:} batchs".format(
                    key, len(value)
                )
            )
...
def train_single_model(
    ...
    for _is, seed in enumerate(seeds):
        ...
        if to_save_name.exists():
            logger.log(
                "Find the existing file {:}, directly load!".format(
                    to_save_name
                )
            )
