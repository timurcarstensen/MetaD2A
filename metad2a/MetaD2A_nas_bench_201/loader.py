...
def get_meta_train_loader(
    ...
    dataset = MetaTrainDatabase(
        data_path=data_path, num_sample=num_sample, is_pred=is_pred
    )
...
def get_meta_test_loader(
    data_path, data_name, num_class=None, is_pred=False
):
    dataset = MetaTestDataset(data_path, data_name, num_class)
    print(f"==> Meta-Test dataset {data_name}")

    loader = DataLoader(
        dataset=dataset, batch_size=100, shuffle=False, num_workers=1
    )
    return loader
...
class MetaTrainDatabase(Dataset):
    def __init__(
        ...
        self.task_lst = torch.load(
            os.path.join(data_path, "meta_train_task_lst.pt")
        )
...
class MetaTestDataset(Dataset):
    def __init__(self, data_path, data_name, num_sample, num_class=None):
        ...
        self.x = torch.load(
            os.path.join(data_path, f"{data_name}bylabel.pt")
        )
