...
def load_graph_config(graph_data_name, nvt, data_path):
    ...
    ms = torch.load(os.path.join(data_path, f"{graph_data_name}.pt"))[
        "arch"
    ]["matrix"]
    ...
    graph_config["num_vertex_type"] = (
        nvt  # original types + start/end types
    )
...
def is_valid_NAS201(g, START_TYPE=0, END_TYPE=1):
    ...
    res = res and 0 not in g.vs["type"][1:-1]
    res = res and 1 not in g.vs["type"][1:-1]
    return res
