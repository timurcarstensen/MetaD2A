...
from metad2a.MetaD2A_nas_bench_201.nas_bench_201 import train_single_model
...
class Predictor:
    ...
    def meta_train(self):
        ...
        for epoch in range(1, self.max_epoch + 1):
            ...
            if self.max_corr_dict["corr"] < vacorr:
                ...
                save_model(
                    epoch, self.model, self.model_path, max_corr=True
                )
    ...
    def meta_train_epoch(self, epoch):
        ...
        return trloss / dlen, pearsonr(
            np.array(y_all), np.array(y_pred_all)
        )[0]
    ...
    def train_single_arch(self, data_name, arch_str, meta_test_path):
        ...
        for seed in seeds:
            result = torch.load(
                os.path.join(meta_test_path, f"seed-0{seed}.pth")
            )
            test_acc_lst.append(
    ...
    def select_top_arch_acc(self, data_name, y_pred_all, gen_arch_str, N):
    ...
    def select_top_arch(self, data_name, y_pred_all, gen_arch_str, N):
        _, sorted_idx = torch.sort(y_pred_all, descending=True)
        sotred_gen_arch_str = [gen_arch_str[_] for _ in sorted_idx]
