...
def load_graph_config(graph_data_name, nvt, data_path):
    if graph_data_name != "ofa_mbv3":
        raise NotImplementedError(graph_data_name)
    max_n = 20
    graph_config = {}
    graph_config["num_vertex_type"] = (
        nvt + 2
    )  # original types + start/end types
...
def decode_ofa_mbv3_to_igraph(matrix):
    ...
    for i in range(num_stage):
        for j in range(num_layer):
            d.append(matrix["d"][i])
    for i, (ks, e, d) in enumerate(zip(matrix["ks"], matrix["e"], d)):
...
class Log:
    ...
    def print_pred_log(
        self, loss, corr, tag, epoch=None, max_corr_dict=None
    ):
        if tag == "train":
            ct = time.time() - self.ep_sttime
            tt = time.time() - self.stime
